...
class DMIAD(nn.Module):
    ...
    Architecture:
    Input → WideResNet Backbone → Feature Projection → Dual Memory → Fusion → Decoder → Output
    """
    ...
    def __init__(self, config):
        ...
        self.feature_dim = config.MODEL.MEMORY.FEATURE_DIM
        
        # === 1. Backbone Network (WideResNet Feature Extractor) ===
        ...
        # Project backbone features to efficient dimensions
        self.conv_proj = nn.Conv2d(4096, self.feature_dim, kernel_size=1, bias=False)
        
        # Project skip connection features
        self.conv_s4 = nn.Conv2d(256, 256, kernel_size=1, bias=False)  # s4 features
        self.conv_s2 = nn.Conv2d(128, 128, kernel_size=1, bias=False)  # s2 features
        ...
        temp_config = type('TempConfig', (), {
            ...
            'MEMORY_DIM': config.MODEL.MEMORY.get('MEMORY_DIM', 256),
            'SHRINK_THRES': config.MODEL.MEMORY.SHRINK_THRES,
            'USE_SHARED_MLP': config.MODEL.MEMORY.get('USE_SHARED_MLP', False),
            'NORMALIZE_MEMORY': config.MODEL.MEMORY.get('NORMALIZE_MEMORY', True),
            'NORMALIZE_QUERY': config.MODEL.MEMORY.get('NORMALIZE_QUERY', True)
        })()
        ...
        if self.use_spatial_memory:
            spa_config = type('SpaConfig', (), {
                ...
                'MEMORY_DIM': config.MODEL.MEMORY.get('MEMORY_DIM', 256),
                'SHRINK_THRES': config.MODEL.MEMORY.SHRINK_THRES,
                'USE_CHANNEL_WISE': config.MODEL.MEMORY.get('USE_CHANNEL_WISE', True),
                'USE_POS_ENCODING': config.MODEL.MEMORY.get('USE_POS_ENCODING', True),
                'NORMALIZE_MEMORY': config.MODEL.MEMORY.get('NORMALIZE_MEMORY', True),
                'NORMALIZE_QUERY': config.MODEL.MEMORY.get('NORMALIZE_QUERY', True),
                'USE_SHARED_MLP': config.MODEL.MEMORY.get('USE_SHARED_MLP', False)
            })()
